...
from src.commands.def_untar import untar

# Добавляем путь к src
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'src'))

def test_untar_not_a_tar(fs: FakeFilesystem):
...
"""def test_untar_right_args(fs: FakeFilesystem):

    fs.create_dir("test_dir")


    with fs.open("test_dir.tar", 'wb') as f:
...
def test_untar_FileNotFoundError(fs: FakeFilesystem):

    fs.create_dir("test_dir")

    with patch('src.commands.def_untar.logger') as mock_logger:
...
def test_untar_PermissionError(fs: FakeFilesystem):
...
def test_untar_Exception(fs: FakeFilesystem):
...
# python -m pytest tests/test_untar.py -v
